...
def init_chatbot():
    ...
    try:
        # Initialize OpenAI client with LM Studio settings
        client = OpenAI(base_url="http://localhost:1234/v1", api_key="lm-studio")
        
        # Configure paths and models
        pdf_path = Path('D:/PBL5-Promotion/tu_van_combo_do_an.pdf')
        persist_dir = Path(__file__).parent / 'static/bot/vector_db'
        embedding_model = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"

        # Initialize vector store if not already done
        if not hasattr(init_chatbot, 'store'):
            ...
            for doc in documents:
                ...
                texts.extend(splits)
                metadatas.extend([{"source": getattr(doc, "metadata", {}).get("source", "")}] * len(splits))
            ...
            init_chatbot.retriever = store.as_retriever(search_kwargs={"k": 5})
            init_chatbot.client = client

        return init_chatbot.client, init_chatbot.retriever
...
chatbot_client, vector_store = init_chatbot()
...
def bot_chat(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Verify chatbot services
            if not chatbot_client or not vector_store:
                logger.error("Chatbot services unavailable")
                return JsonResponse({
                    'error': 'Hệ thống chatbot tạm thời không khả dụng. Vui lòng thử lại sau.'
                }, status=503)

            try:
                # Use get_relevant_documents instead of query
                docs = vector_store.get_relevant_documents(question)
                if not docs:
                    return JsonResponse({
                        'reply': 'Xin lỗi, tôi không tìm thấy thông tin liên quan.'
                    })
                ...
                try:
                    response = chatbot_client.chat.completions.create(
                        ...
                        messages=[{
                            "role": "user", 
                            "content": f"Bạn là một trợ lý AI chỉ được phép trả lời dựa trên nội dung tài liệu bên dưới. "
                                        f"Nếu câu hỏi không liên quan hoặc tài liệu không chứa thông tin, hãy trả lời: "
                                        f"'Tôi không hỗ trợ cho câu hỏi này.'\n\n"
                                        f"Tài liệu:\n{context}\n\n"
                                        f"Câu hỏi: {question}"
                        }],
                        timeout=30
                    ...
                    return JsonResponse({
                        'error': 'Lỗi xử lý câu trả lời. Vui lòng thử lại sau.'
                    }, status=500)
                ...
                return JsonResponse({
                    'error': 'Có lỗi xử lý. Vui lòng thử lại sau.'
                }, status=500)
    ...
    # GET request - show chat interface with error handling
    try:
        recent_msgs = BotMessage.objects.filter(
            user=request.user
